...
import 'soal_harian_pertemuan_screen.dart'; // <-- ganti import ini

class SoalHarianFormScreen extends StatefulWidget {
  final String idMateri;

  const SoalHarianFormScreen({
    super.key,
...
class _SoalHarianFormScreenState extends State<SoalHarianFormScreen>
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Stack(children: [
                        ...
                        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Text('Data ini digunakan untuk mencatat hasil soal harian kamu.',
                    ...
                    Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        _buildField(controller: nisController, label: 'NIS',
                            hint: 'Masukkan NIS kamu', icon: Icons.badge_rounded,
                            keyboardType: TextInputType.number, isTablet: isTablet,
                            validator: (v) => v!.isEmpty ? 'NIS wajib diisi' : null),
                        const SizedBox(height: 14),
                        _buildField(controller: namaController, label: 'Nama Siswa',
                            hint: 'Masukkan nama lengkap', icon: Icons.person_rounded,
                            isTablet: isTablet,
                            validator: (v) => v!.isEmpty ? 'Nama wajib diisi' : null),
                        const SizedBox(height: 14),
                        _buildDropdownKelas(isTablet),
                        const SizedBox(height: 14),
                        _buildField(controller: absenController, label: 'No Absen',
                            hint: 'Masukkan nomor absen', icon: Icons.format_list_numbered_rounded,
                            keyboardType: TextInputType.number, isTablet: isTablet,
                            validator: (v) => v!.isEmpty ? 'No Absen wajib diisi' : null),
                      ]),
                    ),
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(
                          child: Text(
                            'Pilih pertemuan yang sesuai setelah mengisi data diri.',
                            style: TextStyle(fontSize: 12,
                                color: const Color(0xFF2E7D52), height: 1.4),
                          ),
                        ),
                      ]),
                    ),
                    ...
                    // ── TOMBOL LANJUT (ke pilih pertemuan) ──
                    GestureDetector(
                      onTap: () {
                        if (_formKey.currentState!.validate() && selectedKelas != null) {
                          Navigator.push(
                            context,
                            PageRouteBuilder(
  ...
  Widget _buildDropdownKelas(bool isTablet) {
    return DropdownButtonFormField<String>(
      ...
      items: kelasList.map((k) => DropdownMenuItem(
        value: k,
        child: Text(k, style: const TextStyle(fontWeight: FontWeight.w600, color: _dark)),
      )).toList(),
    );
  }
